...
void Draw_Poly ( clipped_polygon_type *clip, u32 *screen )
{
	...
	//vertex_type tempvert;
	
	vars.numverts = clip->number_of_vertices;
	
	vars.edge_count = vars.numverts-1;
	...
	//Font_itoa(clip->vertex[0].y, szout, 10 );
	//Gba_Print(10,20,szout,screen);
	//Font_itoa(clip->vertex[1].y, szout, 10 );
	//Gba_Print(10,40,szout,screen);
	//Font_itoa(clip->vertex[2].y, szout, 10 );
	//Gba_Print(10,60,szout,screen);

	for ( vars.counter = 1; vars.counter < vars.numverts; vars.counter++ )
	{
		s32 tempy;
		tempy = clip->vertex[vars.counter].y; 
	...
	if ( vars.end_vert1 < 0 )
	{
		vars.end_vert1 = clip->number_of_vertices-1;
	}
	...
	if ( vars.end_vert2 == (clip->number_of_vertices))
	{
		vars.end_vert2 =0;
	}
	...
	//240*ystart = 128*y + 64*y + 32*y + 16*y
	//240*ystart = y<<7  + y<<6 + y<<5 + y<<4
...
void Project_Object( object_type *object, s32 distance )
{
	...
	for ( v=0; v<(*object).number_of_vertices; v++) { // Loop through vertices
		...
		z=(ABS(vptr->az));
		vptr->ax=distance*vptr->ax/(distance-z)+XORIGIN;  // ...divide world x&y coords
		vptr->ay=distance*vptr->ay/(distance-z)+YORIGIN;  // ... by z coordinates
	}
